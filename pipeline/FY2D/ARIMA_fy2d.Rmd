...
df <- read.csv("../data_arima/fy2d_mean_motion_data.csv", stringsAsFactors = FALSE) # already done interploate
...
```{r}
# check fitted for selected model
cat("=== Residual Diagnostics ===\n")
checkresiduals(best_fit)  # Ljung‐Box + ACF +  normality QQ
```

```{r}
# fit model with searched p d q
fit_final <- Arima(y,
                   order            = c(4,0,3),
                   include.constant = TRUE,
                   method           = "ML",
...
write.csv(
  df_fitted,
  file        = "../data_arima/fy2d_arima_predictions_with_dates.csv",
  row.names   = FALSE,
  fileEncoding = "UTF-8"
)
```
